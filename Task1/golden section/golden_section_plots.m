% Plots needed for the golden section method
...
for lambda=linspace(0.005,0.15,size) 
    i = i + 1;
    [~,~,k(i),l(i)] = golden_section(f_1,lambda);
end
subplot(3,1,1)
...
for lambda=linspace(0.005,0.15,size)
    i = i + 1;
    [~,~,k(i),l(i)] = golden_section(f_2,lambda);
end
subplot(3,1,2)
plot(l,k,'-r','LineWidth',1.2)
...
for lambda=linspace(0.005,0.15,size)
    i = i + 1;
    [~,~,k(i),l(i)] = golden_section(f_3,lambda);
end
subplot(3,1,3)
plot(l,k,'-r','LineWidth',1.2)
...
for n=1:3
subplot(3,1,n)
[a,b,k] = golden_section(F(j),lam(n));
for i=0:1:k-1
    plot(i,a(i+1),'*r')
    hold on
    plot(i,b(i+1),'ob')
